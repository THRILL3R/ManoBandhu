...
const mergeConfigs = (...configs: object[]) => configs.reduce((acc: any, c: any) => ({ ...acc, ...c }), {});
const getGradientDirection = (position: string) =>
    ({ top: 'to top', bottom: 'to bottom', left: 'to left', right: 'to right' })[position as keyof object] || 'to bottom';
...
function GradualBlur(props: any) {
    ...
    const containerStyle = useMemo(() => {
        ...
        if (isVertical) {
            (baseStyle as any).height = responsiveHeight;
            (baseStyle as any).width = responsiveWidth || '100%';
            (baseStyle as any)[config.position] = 0;
            baseStyle.left = 0;
            baseStyle.right = 0;
        } else if (isHorizontal) {
            (baseStyle as any).width = responsiveWidth || responsiveHeight;
            (baseStyle as any).height = '100%';
            (baseStyle as any)[config.position] = 0;
            baseStyle.top = 0;
            baseStyle.bottom = 0;
        }
